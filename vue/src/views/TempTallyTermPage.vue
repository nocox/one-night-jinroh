...
export default {
    ...
    mounted() {
        axios.get('http://ec2-52-198-98-214.ap-northeast-1.compute.amazonaws.com:8080/tally-index', {withCredentials: true})
        .then((response) => {
    ...
    methods: {
        ...
        gotoResult: function() {
            axios.get('http://ec2-52-198-98-214.ap-northeast-1.compute.amazonaws.com:8080/show-result',{withCredentials: true})
            .then((response) => {
                console.log(response.data);
            }).catch(() => {
                this.$router.push('/temp-room');
